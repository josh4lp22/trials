...
class Zend_View_Helper_Image extends Zend_View_Helper_Abstract
{
	...
	protected function _setAttributes($attribs)
	{
		...
		if (isset($attribs['height'])) {
			$height = 'height="' . $this->view->escape($attribs['height']) . '" ';
		}
		
		if (isset($attribs['width'])) {
			$width = 'width="' . $this->view->escape($attribs['width']) . '" ';
		}
		...
		$this->_attribs = $alt . $title . $map . $class . $height . $width ;
		return $this;
	}
	...
	protected function _setAction($actionCallback)
	{
		...
		$actionClass = 'Zend_View_Helper_Image_Action_' . ucfirst($action);
		$actionClass = new $actionClass($options);

		// check if action class implement 'Moo_View_Helper_Html_Image_ActionInterface'
		if (!$actionClass instanceof Zend_View_Helper_Image_ActionInterface) {
			return false;
		}
